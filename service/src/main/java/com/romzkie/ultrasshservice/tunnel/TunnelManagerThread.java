...
ServerHostKeyVerifier, DebugLogger {
	...
	private synchronized void interruptPinger() {
		if (pinger != null && pinger.isAlive()) {
			SkStatus.logInfo("Đang Dừng Pinger");
	...
	protected void startClienteSSH() throws Exception {
		...
		try {
			...
			//SkStatus.logInfo("<html><font color='#0062AF'>" + mContext.getString(R.string.state_connected) + "</font></html>");
	...
	protected void conectar(String servidor, int porta) throws Exception {
		...
		try {
			...
			if (mConfig.ssh_compression()){
				...
				SkStatus.logInfo("Đã Bật Nén SSH");
			}
			...
			if (useProxy && cause.contains("Key exchange was not finished")) {
				SkStatus.logError("Proxy: Connection Lost");
                            ...
                            SkStatus.logError("Payload: ezTlA55kvBy0IdG0f0IjBfN4lodG9Z2+9rm1nB2pY+Rb73agrgmn6EqqUeWgsxhH/uUI1OgVvXnSOE46uSe1LuPDJLvRuv478ibPQF5TqurRUx+c9Zk/zd5lRfnQcAGFshGSNFJ4Ry71c1mjZrOMawXuce5ctO45w7Ybg2ajknl6cSlWgRIe0PAL5OG1oZkxmmeGLfU3Rtp2lLeCB8KkhKEKEAYnMYezuYPhmzHxg24= " /*+ cause*/);
			}
	...
	protected void autenticar(String usuario, String senha, String keyPath) throws IOException {
		...
		try {
			if (mConnection.isAuthMethodAvailable(usuario,
				...
				SkStatus.logInfo("Đang Xác Thực");
		...
		if (!mConnection.isAuthenticationComplete()) {
			SkStatus.logInfo("Không Xác Thực Được, Tên Người Dùng Hoạc Mật Khẩu Hết Hạn");
	...
	throws Exception {
		...
		SkStatus.logInfo("Mã Finger: " + fingerPrint);
	...
	private void startPinger(final int timePing) throws Exception {
		...
		//SkStatus.logInfo("Đang Bắt Đầu Pinger");
	...
	public void reconnectSSH() {
		...
		for (int i = 0; i < RECONNECT_TRIES; i++) {
			...
			int sleepTime = 5;
			if (!TunnelUtils.isNetworkOnline(mContext)) {
				SkStatus.updateStateString(SkStatus.SSH_AGUARDANDO_REDE, "Chờ Kết Nối Mạng..");
				...
				try {
					...
					SkStatus.logInfo("<html><font color=\"red\">" + mContext.getString(R.string.state_disconnected) + "</font></html>");
				}
	...
	public void onReceiveInfo(int id, String msg) {
		if (id == SERVER_BANNER) {
			SkStatus.logInfo("<font color=#e14681>HÀ LONG VŨ - VPN</font>");
		}
	}
	...
	protected void startTunnelVpnService() throws IOException {
		...
		SkStatus.logInfo("Bắt Đầu Dịch Vụ Đường Hầm");
		SharedPreferences prefs = mConfig.getPrefsPrivate();
		...
		if (mContext.startService(startTunnelVpn) == null) {
			SkStatus.logInfo("Không Thể Khởi Động Dịch Vụ VPN");
	...
	protected synchronized void stopTunnelVpnService() {
		...
		SkStatus.logInfo("Dừng Dịch Vụ Đường Hầm");
