...
public class Tunnel
{
	...
	{
		StringBuilder routeMessage = new StringBuilder("Tuyến Đường: ");
		StringBuilder routeExcludeMessage = new StringBuilder("Loại Trừ: ");
		...
		final String errorMessage = "Khởi Động VPN Không Thành Công";
		try
		{
	...
	private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns)
	{
		...
		if (forwardDns) {
			...
			mPdnsd.setOnPdnsdListener(new Pdnsd.OnPdnsdListener(){
				@Override
				public void onStart(){
					//mHostService.onDiagnosticMessage("Đã Bắt Đầu PDNSD");
				}
				@Override
				public void onStop(){
					//mHostService.onDiagnosticMessage("Đã Dừng PDNSD");
					stop();
				}
			});
		...
		mTun2Socks.setOnTun2SocksListener(new Tun2Socks.OnTun2SocksListener(){
			...
			public void onStart()
			{
				//mHostService.onDiagnosticMessage("Đã Bắt Đầu Dịch Vụ Đường Hầm");
			}
			@Override
			public void onStop()
			{
				//mHostService.onDiagnosticMessage("Đã Dừng Lõi Đường Hầm");
				stop();
			}
		});
		...
		//mHostService.onDiagnosticMessage("Định Tuyến Đường Qua Đường Hầm");
	...
	private void stopVpn()
	{
		...
		if (tunFd != null)
		{
			try
			{
				//mHostService.onDiagnosticMessage("Đã Đã Dừng Dịch Vụ VPN");
				tunFd.close();
			}
			catch (IOException e) {}
		}
