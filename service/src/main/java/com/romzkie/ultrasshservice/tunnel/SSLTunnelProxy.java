...
public class SSLTunnelProxy implements ProxyData
{
	class HandshakeTunnelCompletedListener implements HandshakeCompletedListener {
        ...
        public void handshakeCompleted(HandshakeCompletedEvent handshakeCompletedEvent) {
            ...
            //SkStatus.logInfo(new StringBuffer().append("SSL: Sử Dụng Giao Thức: <br>").append(Arrays.toString(val$sslSocket.getSupportedProtocols())).toString().replace("[", "").replace("]", "").replace(",", "<br>"));
			//SkStatus.logInfo(new StringBuffer().append("SSL: Kích Hoạt Giao Thức: <br>").append(Arrays.toString(val$sslSocket.getEnabledProtocols())).toString().replace("[", "").replace("]", "").replace(",", "<br>"));
        ...
        try {
            ...
            if (sSLContext.getSocketFactory() instanceof android.net.SSLCertificateSocketFactory && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN_MR1) {
				...
				try {
					...
					SkStatus.logInfo("Thiết Lập SNI...");
				} catch (Throwable e) {
