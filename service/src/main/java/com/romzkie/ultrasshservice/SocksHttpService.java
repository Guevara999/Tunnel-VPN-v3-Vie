...
implements SkStatus.StateListener
{
    ...
    public static final String START_SERVICE = "com.vienetworks.originvpn:startTunnel";
	...
	public synchronized void startTunnel() {
		...
		SkStatus.logInfo(String.format("Địa Chỉ IP: %s", getIpPublic()));
        
        if(mPrefs.getWakelock()){
            wakeLock.acquire();
            wakeLock.release(); 
            SkStatus.logInfo("Đã Khóa CPU");
		...
		try {
			...
			//SkStatus.logInfo("Bắt Đầu Đường Hầm");
	...
	public synchronized void stopTunnel() {
		if (mTunnelManager != null) {
			...
			if (mTunnelThread != null) {
				...
				//SkStatus.logInfo("Đường Hầm Dừng Lại");
			}
    ...
    public void onLowMemory() {
        ...
        SkStatus.logWarning("Cảnh Báo Bộ Nhớ Thấp!");
    }
	...
	protected void networkStateChange(Context context, boolean showStatusRepetido) {
		...
		try {
			...
			if (networkInfo == null) {
				...
				netstatestring = String.format("Kết Nối Mạng %4$s Đến %1$s %3$s", networkInfo.getTypeName(),
