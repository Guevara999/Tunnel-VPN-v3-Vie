...
public class Connection
{
	...
	public static class Core extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... args) {
			try {
				String api = new StringBuilder().append("https://pastegen.com/raw/0RpuR54bXB").toString();
				URL url = new URL(api);
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("error")) {
				...
				try
				{
					JSONArray jarr = new JSONObject(result).getJSONArray("Blocked");
					for (int i = 0; i < jarr.length(); i++)
					{
						//JSONObject obj = jarr.getJSONObject(i);
						if(jarr.getString(i).equals(context.getPackageName())){
							TunnelManagerHelper.stopSocksHttp(context);
							new AlertDialog.Builder(context)
								.setTitle("App Disabled")
								.setMessage("App Disabled by the real developer!")
								.setCancelable(false)
								.create().show();
							break;
						}
