...
public class ConfigUtil {
    ...
    public static final String SocksHttpMainActivity = "longvuvpnplus";
    String CONFIGPASSWORD = ("longvuvpnplus");
    public ConfigUtil(Context context) {
        this.context = context;
    }
	public String getNote() {
        try {
            String note = getJSONConfig().getString("ReleaseNotes");
            return note;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private JSONObject getJSONConfig() {
        try {
            ...
            if (file.exists()) {
                ...
                String json = AESCrypt.decrypt(SocksHttpMainActivity, json_file);
                return new JSONObject(json);
            } else {
                InputStream inputStream = context.getAssets().open("config/config.json");
                String json = AESCrypt.decrypt(SocksHttpMainActivity, readStream(inputStream));
                return new JSONObject(json);
            }
        } catch (Exception e) {
            e.printStackTrace();
